class TourPdf < Prawn::Document
  ...
  def initialize(tour, view)
    ...
    text "#{I18n.t('Report Created at')} #{@tour.created_at.strftime("%d %b %Y")}", :size => 8, :style => :italic, :align => :left
    logo
    ...
    text_box "#{I18n.t('Form_field_four')} #{@tour.percent ? "1,5%" : "2%"}", :size => 7, :color => '020255',
      :at => [10,540]
    ...
    text_box "#{@tour.name}", :at => [x_1 + 20, y_1 - 1], :width =>  width - 5
    ...
    text_box "#{@tour.fullName}", :at => [x_2 - 10, y_2 - 1], :width => 120
    ...
    text_box "#{@tour.totalAmount.to_money.format} #{I18n.t('Hr')}", :at => [x_3 + 20, y_3 - 1], :width =>  width - 5
    ...
    text_box "#{@tour.commission.to_money.format} #{I18n.t('Hr')}", :at => [x_4 + 20, y_4 - 1], :width =>  width - 5
